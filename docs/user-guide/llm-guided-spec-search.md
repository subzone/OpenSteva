...
```bash
python examples/opensteva/spec_search_quickstart.py
```
...
`opensteva.learning.spec_search.composite_reward.score_batch`.
...
`configs/opensteva/examples/spec-search-quickstart.toml`. Copy it to
`~/.opensteva/config.toml` (or set `OPENJARVIS_CONFIG` to it) and the
...
| `trace_store` | `MagicMock` | `opensteva.traces.store.TraceStore(home / "traces.db")` |
| `student_runner` | `MagicMock` | `opensteva.learning.spec_search.student_runner.VLLMStudentRunner(host=..., model=...)` |
| `judge` | `MagicMock` | `opensteva.evals.core.scorer.LLMJudgeScorer(...)` (or a deterministic scorer if your benchmark provides one) |
| `session_store` | `MagicMock` | `opensteva.learning.spec_search.storage.session_store.SessionStore(home / "learning" / "sessions.db")` |
| `checkpoint_store` | `MagicMock` | `opensteva.learning.spec_search.checkpoint.store.CheckpointStore(home / "learning" / "checkpoints")` |
...
1. Create `src/opensteva/evals/datasets/<corpus>.py` implementing
   `DatasetProvider` (`adp.py` is a small reference). The provider's
   `load(max_samples, seed, split)` must respect `split` via
   `apply_split` from `opensteva.evals.core.splits`.
2. Register: `@DatasetRegistry.register("<corpus>")`.
3. Feed it to the proposer via the trace store:

```python
from opensteva.evals.datasets.adp import ADPDataset
from opensteva.learning.spec_search.external_adapter import (
...
store = TraceStore("~/.opensteva/traces.db")
...
`src/opensteva/evals/backends/jarvis_agent.py` previously hardcoded
`builder.telemetry(telemetry).traces(True).build()`, ignoring the
`telemetry` parameter. This silently caused every agent-backend
evaluation to write to `~/.opensteva/traces.db` regardless of caller
...
- `examples/opensteva/spec_search_quickstart.py` — runnable end-to-end demo.
- `configs/opensteva/examples/spec-search-quickstart.toml` — prebuilt config.
- `src/opensteva/learning/spec_search/orchestrator.py` — `SpecSearchOrchestrator` (single session).
- `src/opensteva/learning/spec_search/multi_session.py` — `SpecSearchLoop` (Algorithm 1).
- `src/opensteva/learning/spec_search/composite_reward.py` — paper Eq. 1.
- `src/opensteva/learning/spec_search/gate/benchmark_gate.py` — `GateOK` predicate.
- `src/opensteva/learning/spec_search/external_adapter.py` — corpus → trace adapter.
