...
Agents are the agentic logic layer of OpenSteva. They determine how a query is processed -- whether it goes directly to a model, through a tool-calling loop, via ReAct reasoning, CodeAct code execution, recursive decomposition, or an external agent runtime. All agents implement the `BaseAgent` ABC and are registered via the `AgentRegistry`.
...
!!! note "Backward compatibility"
    The registry alias `"react"` maps to `NativeReActAgent`. The old import `from opensteva.agents.react import ReActAgent` also still works.
...
!!! warning "Requirements"
    Requires Node.js 22+ on `PATH` and an `ANTHROPIC_API_KEY` environment variable (or pass `api_key=` directly). The bundled runner is auto-installed to `~/.opensteva/claude_code_runner/` on first use via `npm install`.

**How it works:**

1. On first call, copies the bundled `claude_code_runner/` to `~/.opensteva/claude_code_runner/` and runs `npm install --production` if `node_modules` is missing.
...
**When to use:** For software engineering tasks where the Claude Agent SDK's built-in tools (code editing, bash execution, file operations) provide capabilities beyond what OpenSteva tool-calling agents support.

```python
from opensteva.agents.claude_code import ClaudeCodeAgent
...
!!! info "accepts_tools = False"
    `ClaudeCodeAgent` does not accept OpenSteva tools via `--tools`. Tool access for the Claude agent is configured separately via the `allowed_tools` constructor parameter, which passes tool names understood by the Claude Agent SDK itself.

---

## OpenCodeAgent

The `OpenCodeAgent` delegates coding tasks to [opencode](https://opencode.ai), the open-source coding agent, running it **on your local engine**. opencode handles the agentic loop, file edits, and tool use; OpenSteva supplies the model — keeping coding-agent work local-first.

!!! warning "Requirements"
    Requires the `opencode` binary on `PATH` (`npm i -g opencode-ai` or `brew install anomalyco/tap/opencode`). It is **not** bundled; `run()` returns a clear error if it is missing. No `ANTHROPIC_API_KEY` needed — inference goes through your OpenSteva engine.

**How it works:**

1. Derives an OpenAI-compatible base URL from the `engine` (e.g. Ollama/vLLM/llama.cpp at `<host>/v1`) and writes an `opencode.json` in the workspace registering it as an `@ai-sdk/openai-compatible` provider (`opensteva/<model>`).
...
| `provider_id`       | `str`             | `"opensteva"`   | opencode provider id to register/use                     |
...
```python
from opensteva.agents.opencode import OpenCodeAgent
...
```python
from opensteva.agents.operative import OperativeAgent
...
```python
from opensteva.agents.monitor_operative import MonitorOperativeAgent
...
```python
from opensteva.sandbox import ContainerRunner, SandboxedAgent
from opensteva.agents.simple import SimpleAgent

runner = ContainerRunner(
    image="opensteva-sandbox:latest",
    timeout=60,
    mount_allowlist_path="/etc/opensteva/mount_allowlist.json",
)
...
| Parameter              | Type   | Default                      | Description                                    |
|------------------------|--------|------------------------------|------------------------------------------------|
| `image`                | `str`  | `"opensteva-sandbox:latest"`| Docker image to run                            |
