...
OpenSteva has two types of integrations:
...
```json
{
    "display_information": { "name": "OpenSteva" },
    "features": {
        "app_home": {
        ...
        "bot_user": { "display_name": "OpenSteva", "always_online": true }
    },
    "oauth_config": {
        "scopes": {
            "bot": [
...
| Bot not visible in Slack | Click "+" next to Direct Messages → search "OpenSteva" |
...
Slack serves two purposes in OpenSteva:
...
8. **(Optional) Set the app icon:**
   ...
   - Upload the [OpenSteva icon](https://github.com/subzone/OpenSteva/blob/main/assets/opensteva-slack-icon.jpg)
...
**DM your agent:**
- In Slack, find **OpenSteva** under Apps (or Direct Messages)
- If you don't see it: click "+" next to Direct Messages → search "OpenSteva"
...
- **Channel visibility:** The bot can only read channels it's been added to. Invite it with `/invite @OpenSteva` in each channel you want indexed.
...
| No messages found (data source) | The bot can only see channels it's been added to. Invite it: `/invite @OpenSteva` in the channel |
...
1. **Grant Full Disk Access** to your terminal app:
   ...
   - Enable access for Terminal, iTerm, Warp, or the OpenSteva desktop app
...
| Want to reset a connector | Click "Reconnect" in the Channels tab, or delete the credential file at `~/.opensteva/connectors/{connector}.json` |

### Where credentials are stored

All credentials are saved locally at `~/.opensteva/connectors/` with file permissions `0600` (owner-only read/write). No credentials are sent to any server — everything stays on your device.

```
~/.opensteva/connectors/
