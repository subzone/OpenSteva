...
Channel implementations use Python's `threading` module rather than asyncio. This is a deliberate choice: OpenSteva's core inference path is synchronous, and daemon threads are simpler to compose with synchronous code than coroutines.
...
1. Create a new file in `src/opensteva/channels/`.
...
```python
from opensteva.channels._stubs import BaseChannel, ChannelMessage, ChannelStatus
...
- [API Reference: Channels](../api-reference/opensteva/channels/index.md) -- complete class and type signatures
