...
!!! note "Backward compatibility"
    The canonical locations are now `opensteva.learning._stubs` (for `RouterPolicy` and `QueryAnalyzer`) and `opensteva.core.types` (for `RoutingContext`). The old `opensteva.intelligence._stubs` import path still works via a backward-compatibility shim, but new code should import from `opensteva.learning._stubs`.
...
```python
from opensteva.learning.router import HeuristicRouter, build_routing_context
...
```python
from opensteva.learning.router import build_routing_context
...
```python
from opensteva.learning.trace_policy import TraceDrivenPolicy
from opensteva.traces.analyzer import TraceAnalyzer
...
```python
from opensteva.learning.sft_policy import SFTRouterPolicy
# or via the backward-compat alias:
from opensteva.learning.sft_policy import SFTPolicy
```
...
```python
from opensteva.learning.agent_advisor import AgentAdvisorPolicy
```
...
```python
from opensteva.learning.icl_updater import ICLUpdaterPolicy
```
...
```python
from opensteva.learning.heuristic_reward import HeuristicRewardFunction
...
over OpenSteva's 5-primitive configuration space. It automates finding optimal
...
The optimization framework has full Rust parity via the `opensteva-learning`
