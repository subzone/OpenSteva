...
are local fakes, so you can ``python examples/opensteva/spec_search_quickstart.py``
with no API keys and no Ollama / vLLM running. Every "swap this for production"
hookpoint is called out inline.

Configuration is read from ``configs/opensteva/examples/spec-search-quickstart.toml``
via the regular ``opensteva.core.config.load_config`` machinery — you can copy
that TOML to ``~/.opensteva/config.toml`` and tune the gate / stagnation /
reward knobs without editing this file.

Run:

    OPENJARVIS_HOME=/tmp/opensteva-spec-search-demo \\
...
from opensteva.core.config import SpecSearchLearningConfig
from opensteva.learning.spec_search.composite_reward import (
    RewardWeights,
    TrainingSample,
    score_batch,
)
from opensteva.learning.spec_search.models import (
    BenchmarkSnapshot,
    FailureCluster,
)
from opensteva.learning.spec_search.multi_session import SpecSearchLoop
from opensteva.learning.spec_search.orchestrator import SpecSearchOrchestrator
from opensteva.learning.spec_search.triggers import OnDemandTrigger
...
def _fake_diagnosis() -> Any:
    ...
    from opensteva.learning.spec_search.diagnose.runner import DiagnosisResult
...
def build_orchestrator(
    ...
    return SpecSearchOrchestrator.from_config(
        ...
        # Judge: production = opensteva.evals.core.scorer.LLMJudgeScorer(...)
        judge=MagicMock(),
...
def main() -> None:
    # In a real deployment, ``load_config()`` reads from ``~/.opensteva/config.toml``.
    ...
    #
    # (after copying ``configs/opensteva/examples/spec-search-quickstart.toml``
    # to ``~/.opensteva/config.toml``)
    ...
    home = Path(
        ...
        or tempfile.mkdtemp(prefix="opensteva-spec-search-")
    )
    ...
    with patch(
        "opensteva.learning.spec_search.orchestrator.DiagnosisRunner"
    ) as MockDiag:
