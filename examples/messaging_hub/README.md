...
OpenSteva orchestrator agent.

## What This Demonstrates

- **Channel integration** — connecting OpenSteva to messaging platforms (Slack, WhatsApp, etc.)
...
- Python 3.10+
- OpenSteva installed: `uv sync --extra dev`
...
2. Configure the WhatsApp Baileys bridge (see the OpenSteva channel docs).
...
OpenSteva supports many channels — LINE, Viber, Mastodon, Rocket.Chat, and
...
Refer to `configs/opensteva/config.toml` for the full list of channel and agent
options.
...
this via the OpenSteva scheduler:
...
schedule. See `src/opensteva/recipes/data/operators/` for examples.
