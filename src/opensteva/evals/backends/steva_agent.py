"""Steva Agent backend — agent-level inference with tool calling."""
...
from opensteva.evals.backends._commit_util import opensteva_commit
from opensteva.evals.core.backend import InferenceBackend


class StevaAgentBackend(InferenceBackend):
    """Agent-level inference via SystemBuilder + StevaSystem.ask().

    Supports tool calling via the agent harness. Works for both local
    and cloud models.
    """

    backend_id = "steva-agent"
    framework_name = "opensteva"
    ...
    def __init__(
        ...
        # Override the agent's per-run turn budget. StevaConfig.agent.max_turns
    ...
    def framework_commit_value(self) -> str:
        """OpenSteva repo HEAD commit (for telemetry tagging)."""
        from opensteva.evals.backends._commit_util import opensteva_commit

        return opensteva_commit()
    ...
    def generate_full(
        ...
        *,
        ...
        return {
            ...
            "framework": "opensteva",
            "framework_commit": opensteva_commit(),
            "error": None,
        }
