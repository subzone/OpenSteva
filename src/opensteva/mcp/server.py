"""MCP Server — wraps OpenSteva tools as MCP-discoverable tools."""
...
from opensteva.mcp.protocol import (
    INTERNAL_ERROR,
    INVALID_PARAMS,
    METHOD_NOT_FOUND,
    MCPRequest,
...
class MCPServer:
    """MCP server that exposes OpenSteva tools via JSON-RPC.
    ...
    def _auto_discover_tools() -> List[BaseTool]:
        ...
        try:
            ...
            pass
        try:
            from opensteva.tools.web_search import WebSearchTool

            _tool_classes.append(WebSearchTool)
        except ImportError:
            pass
        try:
            from opensteva.tools.code_interpreter import CodeInterpreterTool

            _tool_classes.append(CodeInterpreterTool)
        except ImportError:
            pass
        try:
            from opensteva.tools.repl import ReplTool
        ...
        # Storage MCP tools
        try:
            from opensteva.tools.storage_tools import (
                MemoryIndexTool,
                MemoryRetrieveTool,
                MemorySearchTool,
                MemoryStoreTool,
        ...
        # Channel MCP tools
        try:
            from opensteva.tools.channel_tools import (
                ChannelListTool,
                ChannelSendTool,
                ChannelStatusTool,
            )
