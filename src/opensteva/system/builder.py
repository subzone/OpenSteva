"""Config-driven fluent builder that wires up a StevaSystem."""
...
from opensteva.core.config import StevaConfig, load_config
from opensteva.core.events import EventBus, get_event_bus
from opensteva.engine._stubs import InferenceEngine
from opensteva.system.core import StevaSystem
from opensteva.tools._stubs import BaseTool, ToolExecutor

logger = logging.getLogger(__name__)


class SystemBuilder:
    """Config-driven fluent builder for StevaSystem."""

    def __init__(
        self,
        config: Optional[StevaConfig] = None,
        *,
    ...
    def build(self) -> StevaSystem:
        """Construct a fully wired StevaSystem."""
        config = self._config
        ...
        from opensteva.security import setup_security
        ...
        if config.agent_manager.enabled:
            try:
                ...
                am_db = config.agent_manager.db_path or str(
                    Path("~/.opensteva/agents.db").expanduser()
                )
        ...
        if speech_enabled:
            try:
                from opensteva.speech._discovery import get_speech_backend
    ...
    def _resolve_engine(self, config: StevaConfig):
        from opensteva.engine._discovery import get_engine
    ...
    def _resolve_model(self, config: StevaConfig, engine: InferenceEngine) -> str:
        if self._model:
            return self._model
    ...
    def _resolve_memory(self, config):
        try:
            import opensteva.tools.storage  # noqa: F401 -- trigger registration
    ...
    def _resolve_channel(self, config, bus):
        ...
        try:
            import opensteva.channels  # noqa: F401 -- trigger registration
            from opensteva.core.registry import ChannelRegistry
            from opensteva.system._channel_kwargs import build_channel_kwargs
    ...
    def _setup_sandbox(self, config):
        ...
        if not sandbox_enabled:
            return None
        try:
            from opensteva.sandbox.runner import ContainerRunner
    ...
    @staticmethod
    def _setup_learning_orchestrator(config: StevaConfig):
        if not config.learning.training_enabled:
            return None
        try:
            from opensteva.core.config import DEFAULT_CONFIG_DIR
            from opensteva.learning.learning_orchestrator import (
                LearningOrchestrator,
            )
            from opensteva.learning.training.lora import LoRATrainingConfig
            from opensteva.traces.store import TraceStore
