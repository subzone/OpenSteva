...
from opensteva.mining._models import get_pearl_model_spec, pearl_variant_for_base_model
...
def detect_for_engine_model(
    *,
    ...
    ``steva mine doctor`` and ``steva mine init``.
    """
    ...
    if spec is None:
        return MiningCapabilities(
            False,
            reason=f"model {model!r} is not in OpenSteva' Pearl model registry",
        )
...
# ---------------------------------------------------------------------------
# Doctor checks (one per row of `steva mine doctor` output)
# ---------------------------------------------------------------------------
