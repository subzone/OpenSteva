"""``steva doctor`` — run diagnostic checks on the OpenSteva installation."""
...
from opensteva.core.config import DEFAULT_CONFIG_PATH, load_config
...
def _check_config_exists() -> CheckResult:
    ...
    return CheckResult(
        ...
        details="Run `steva init` to generate a config file.",
    )
...
def _check_engines() -> List[CheckResult]:
    ...
    from opensteva.engine import _discovery
...
def _check_optional_deps() -> List[CheckResult]:
    ...
    results: List[CheckResult] = []
    optional_packages = [
        ("fastapi", "opensteva[server]", "REST API server"),
        ("torch", "pip install torch", "SFT/GRPO training"),
        ("pynvml", "opensteva[gpu-metrics]", "NVIDIA energy monitoring"),
        ("amdsmi", "opensteva[energy-amd]", "AMD energy monitoring"),
        ("colbert", "opensteva[memory-colbert]", "ColBERT memory backend"),
        ("zeus", "opensteva[energy-apple]", "Apple Silicon energy monitoring"),
    ]
...
def doctor(as_json: bool) -> None:
    """Run diagnostic checks on your OpenSteva installation."""
    ...
    console = Console()
    console.print()
    console.print("[bold]OpenSteva Doctor[/bold]")
    console.print()
    ...
    from opensteva.cli._bg_state import get_status
    ...
    if bg.rust_extension == "ready":
        ...
        console.print(
            "    retry: ~/.opensteva/.scripts/install-rust.sh && "
            "~/.opensteva/.scripts/build-extension.sh"
        )
        bg_failed = True
    else:
        console.print(
    ...
    for model_id, state in bg.models.items():
        if state == "ready":
            ...
            console.print(f"    retry: ~/.opensteva/.scripts/pull-model.sh {model_id}")
            bg_failed = True
        else:
