"""``steva start|stop|restart|status`` — daemon management commands."""
...
from opensteva.core.config import DEFAULT_CONFIG_DIR, load_config
...
@click.group()
def daemon() -> None:
    """Manage the OpenSteva server daemon."""
...
def start(
    ...
    """Start the OpenSteva server as a background daemon."""
    console = Console(stderr=True)

    existing = _read_pid()
    if existing is not None:
        console.print(f"[yellow]Server already running (PID {existing}).[/yellow]")
        console.print("Use 'steva stop' to stop it first, or 'steva restart'.")
        sys.exit(1)
    ...
    # Build command to run steva serve
    cmd = [sys.executable, "-m", "opensteva.cli", "serve"]
    if host:
    ...
    console.print(
        f"[green]OpenSteva server started[/green] (PID {proc.pid})\n"
...
@daemon.command()
def stop() -> None:
    """Stop the running OpenSteva server daemon."""
    console = Console(stderr=True)
    pid = _read_pid()
    if pid is None:
...
def restart(ctx: click.Context) -> None:
    """Restart the OpenSteva server daemon."""
    console = Console(stderr=True)
    pid = _read_pid()
    if pid is not None:
...
@daemon.command()
def status() -> None:
    """Show status of the OpenSteva server daemon."""
    console = Console(stderr=True)
    pid = _read_pid()
    if pid is None:
