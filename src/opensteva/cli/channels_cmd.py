"""``steva channels`` — manage messaging channels for the agent."""
...
def channels_status() -> None:
    ...
    from opensteva.channels.imessage_daemon import is_running
    ...
    if is_running():
        ...
        table.add_row(
            "iMessage",
            "[dim]stopped[/dim]",
            "steva channels imessage-start <contact>",
        )
    ...
    if background:
        ...
        proc = subprocess.Popen(
            [
                sys.executable,
                "-m",
                "opensteva.channels.imessage_daemon",
                "--chat",
                chat_identifier,
            ],
        ...
        from opensteva.connectors.retriever import (
            TwoStageRetriever,
        )
        from opensteva.connectors.store import KnowledgeStore
        from opensteva.engine.ollama import OllamaEngine
        from opensteva.tools.knowledge_search import (
            KnowledgeSearchTool,
        )
        from opensteva.tools.knowledge_sql import (
            KnowledgeSQLTool,
        )
...
def imessage_stop() -> None:
    ...
    from opensteva.channels.imessage_daemon import stop_daemon
