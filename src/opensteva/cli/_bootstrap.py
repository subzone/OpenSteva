...
Used by both ``install.sh`` (via ``steva _bootstrap --write-config``)
and ``steva init`` (so there is a single source of truth for the
...
import opensteva
from opensteva.core import config as _cfg
...
def write_initial_config(
    *,
    ...
    Called by both ``install.sh`` (via ``steva _bootstrap --write-config``)
    and ``steva init`` so the TOML format has one definition.
    """
    ...
    base_toml = (
        f"# OpenSteva configuration\n"
        f"{hardware_line}{gpu_comment}\n"
        f"# Full reference config: steva init --full\n"
        f"\n"
        f"{provenance}\n"
        f"\n"
        f"[engine]\n"
...
# ---------------------------------------------------------------------------
# CLI command — invoked by install.sh, hidden from `steva --help`
# ---------------------------------------------------------------------------
