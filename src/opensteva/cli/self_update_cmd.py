"""`steva self-update` — upgrade OpenSteva to the latest release.

Runs the right upgrade command for how the user installed OpenSteva:

- PyPI installs get ``pip install --upgrade opensteva``.
- uv-tool installs get ``uv tool upgrade opensteva``.
...
import opensteva
from opensteva.cli._install_detect import detect_install


@click.command(
    "self-update",
    help=(
        "Upgrade OpenSteva to the latest release. Detects how you "
...
def self_update(check: bool, yes: bool) -> None:
    info = detect_install()
    current = opensteva.__version__

    click.echo(f"Current OpenSteva version: v{current}")
    ...
    click.echo("\nUpgrade complete. Re-run `steva --version` to confirm.")
