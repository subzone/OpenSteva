"""Bare-`steva` first-run guard.

When the user types ``steva`` with no subcommand, route them to the
chat command if a config exists, otherwise into the init wizard with
the ``--from-bare-steva`` flag (which lets init suppress the
...
def check_and_route(ctx: click.Context) -> None:
    ...
    (the user typed something specific like ``steva ask``).
    """
    if ctx.invoked_subcommand is not None:
        return

    # Late imports to avoid circular import with cli/__init__.py.
    from opensteva.cli.chat_cmd import chat as chat_cmd
    from opensteva.cli.init_cmd import init as init_cmd

    if _cfg.DEFAULT_CONFIG_PATH.exists():
        ctx.invoke(chat_cmd)
    else:
        ctx.invoke(init_cmd, from_bare_steva=True)
