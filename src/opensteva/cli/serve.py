"""``steva serve`` — OpenAI-compatible API server."""
...
from opensteva.cli._banner import print_banner
from opensteva.core.config import load_config
from opensteva.core.events import EventBus
from opensteva.engine import (
    discover_engines,
    discover_models,
    get_engine,
)
from opensteva.intelligence import (
...
def serve(
    ...
    if _has_cloud and engine_name != "cloud":
        try:
            from opensteva.engine.cloud import CloudEngine
            from opensteva.engine.multi import MultiEngine
        ...
        try:
            ...
            if AgentRegistry.contains(agent_key):
                ...
                if getattr(agent_cls, "accepts_tools", False):
                    import opensteva.tools  # noqa: F401  # trigger registration
                    ...
                    from opensteva.mcp.loader import load_mcp_tools_from_config
    ...
    if channel_bridge is not None:
        from opensteva.system import StevaSystem
            ...
            try:
                ...
                if AgentRegistry.contains(channel_agent):
                    ...
                    if getattr(_ch_cls, "accepts_tools", False):
                        ...
                        from opensteva.mcp.loader import (
                        ...
                        # StevaSystem below; we extend its lifetime by
    ...
    if not api_key:
        try:
            ...
            _cfg_path = str(
                __import__("pathlib").Path.home() / ".opensteva" / "config.toml"
            )
    ...
    from opensteva.server.auth_middleware import check_bind_safety

    check_bind_safety(bind_host, api_key=api_key)

    # Log credential status at startup
    from opensteva.core.credentials import TOOL_CREDENTIALS, get_credential_status
    ...
    console.print(
        f"[green]Starting OpenSteva API server[/green]\n"
