"""``steva ask`` — send a query to the assistant."""
...
from opensteva.cli._tool_names import resolve_tool_names
from opensteva.cli.hints import hint_no_engine
...
def _run_research(
    *,
    ...
    from opensteva.agents.research_loop import DEFAULT_PLANNER_MODEL, ResearchAgent
    from opensteva.connectors.embeddings import OllamaEmbedder
    from opensteva.connectors.hybrid_search import HybridSearch
    ...
    # (so ``steva ask --research "..." > out.md`` still gives a clean
...
@click.option(
    ...
    help=(
        ...
        "(default: ~/.opensteva/knowledge.db)."
    ),
)
@click.option(
    "--persona",
    ...
    default=None,
    help=(
        "Named persona dir under ~/.opensteva/personas/<name>/ "
...
def ask(
    ...
    # the most common command (`steva ask "..."`).
    ...
    if resolved is None:
        console.print(
            ...
            "  [cyan]steva config set engine.ollama.host http://<remote-ip>:11434[/cyan]\n"
