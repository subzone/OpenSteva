"""``steva init`` — detect hardware, generate config, write to disk."""
...
from opensteva.cli._bootstrap import detect_cloud_keys
from opensteva.cli.model import find_model_spec, hf_download, ollama_pull
from opensteva.cli.scan_cmd import PrivacyScanner
...
# Engines supported by ``steva init --engine``.
_SUPPORTED_ENGINES = [
    "ollama",
    "vllm",
    "sglang",
...
def _next_steps_text(engine: str, model: str = "") -> str:
    ...
    steps: dict[str, str] = {
        "ollama": (
            ...
            '     steva ask "Hello"\n'
            "\n"
            "  Run `steva doctor` to verify your setup."
        ),
        "vllm": (
            "Next steps:\n"
            "\n"
...
def _quick_privacy_check(console: Console) -> None:
    ...
    console.print()
    console.print("  Run [cyan]steva scan[/cyan] for a full environment audit.")
...
@click.option(
    ...
    default=False,
    hidden=True,
    help="Run init non-interactively; called by the bare-steva first-run guard.",
)
@click.pass_context
def init(
    ctx: click.Context,
    ...
    from_bare_steva: bool = False,
) -> None:
    """Detect hardware and generate ~/.opensteva/config.toml."""
    ...
    if preset:
        examples_dir = (
            Path(__file__).resolve().parents[2] / "configs" / "opensteva" / "examples"
        )
        ...
        console.print(
            ...
            "[bold]steva doctor[/bold] to verify."
        )
        return
    ...
    if engine is None and config is None:
        ...
        # Bare-steva cold path: use the recommended engine non-interactively.
        if from_bare_steva:
            engine = recommended
        else:
            console.print()
...
"""
        ...
        console.print(
            ...
            "Run [bold]steva connect gdrive[/bold] to connect "
            "Google services, then [bold]steva digest --fresh[/bold]."
        )
    ...
    if not soul_path.exists():
        soul_path.write_text(
            "# Agent Persona\n\nYou are Steva, a helpful personal AI assistant.\n"
        )
    ...
    if not model:
        ...
        if not no_download and not from_bare_steva and spec:
            ...
            if click.confirm(prompt, default=True):
                ...
                console.print(
                    ...
                    f"    [bold]steva model pull {model}[/bold]"
                )
