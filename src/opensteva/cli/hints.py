...
def hint_no_config() -> str:
    ...
    return (
        ...
        "  Run [bold]steva init[/bold] to detect hardware and generate "
        "[cyan]~/.opensteva/config.toml[/cyan].\n"
        "  Or run [bold]steva quickstart[/bold] for a guided setup."
    )
...
def hint_no_engine(engine_name: Optional[str] = None) -> str:
    ...
    return (
        ...
        "  Run [bold]steva doctor[/bold] to check all engines.\n"
        "  Run [bold]steva quickstart[/bold] for guided setup.\n"
        "\n"
        "  [dim]To use a remote engine:[/dim]\n"
        f"    [cyan]steva config set engine.{name}.host http://<remote-ip>:<port>[/cyan]\n"
...
def hint_no_model(model_name: Optional[str] = None) -> str:
    ...
    if model_name:
        return (
            ...
            "  Run [bold]steva model list[/bold] to see available models."
        )
    return (
...
def mining_not_running_hint(cfg: object | None, sidecar_present: bool) -> Optional[str]:
    ...
    if cfg is None or sidecar_present:
        return None
    return "mining configured but not running - start it with `steva mine start`"
