"""Check for newer OpenSteva releases on PyPI."""
...
_CACHE_PATH = Path("~/.opensteva/version-check.json").expanduser()
_CACHE_TTL = 86400  # 24 hours
_PYPI_API = "https://pypi.org/pypi/opensteva/json"
...
def _config_path() -> Path:
    ...
    return Path("~/.opensteva/config.toml").expanduser()
...
def _do_check() -> None:
    ...
    try:
        if Version(latest) > Version(current):
            ...
            sys.stderr.write(
                f"\033[33mA new version of OpenSteva is available "
                f"(v{current} → v{latest})\n"
                f"Update: {cmd}\n"
                f"Or run: steva self-update\033[0m\n\n"
            )
