"""``steva host`` — download and serve a model locally with auto backend setup."""
...
def host(
    ...
    \b
      steva host mlx-community/Qwen2.5-7B-4bit --backend mlx
      steva host Qwen/Qwen3-8B --backend vllm
      steva host qwen3:8b --backend ollama
      steva host meta-llama/Llama-3-8B -b sglang
    """
    console = Console()
    ...
    if backend != "ollama":
        ...
        console.print(
            "[dim]OpenSteva will auto-discover it. Press Ctrl+C to stop.[/dim]\n"
        )
