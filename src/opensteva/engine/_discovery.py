...
from opensteva.engine._base import InferenceEngine
...
def _make_engine(key: str, config: StevaConfig) -> InferenceEngine:
...
def _maybe_register_mining_sidecar_engine() -> None:
    ...
    from opensteva.engine._openai_compat import _OpenAICompatibleEngine
...
def discover_engines(config: StevaConfig) -> List[Tuple[str, InferenceEngine]]:
...
def get_engine(
    config: StevaConfig, engine_key: str | None = None
