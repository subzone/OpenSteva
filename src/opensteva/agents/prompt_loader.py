...
def _opensteva_home() -> Path:
    """Resolve $OPENJARVIS_HOME, defaulting to ~/.opensteva."""
    return Path(os.environ.get("OPENJARVIS_HOME", "~/.opensteva")).expanduser()
...
def load_system_prompt_override(agent_name: str) -> str | None:
    ...
    ``OPENJARVIS_HOME`` defaults to ``~/.opensteva`` when unset.
    """
    home = _opensteva_home()
