...
called from your app startup:
    ...
    from opensteva.agents.proactive_agent import register_cron
...
from opensteva.tools.approval_store import (
...
from opensteva.tools.proactive_tools import get_store
...
def _build_notification_channel(channel_spec: str) -> Optional[Any]:
    ...
    if channel_type == "imessage":
        from opensteva.channels._stubs import (
            BaseChannel,
            ChannelStatus,
        )
        ...
        class _IMessageShim(BaseChannel):
            ...
            def send(
                ...
                from opensteva.channels.imessage_daemon import send_imessage
...
class ProactiveAgent(ToolUsingAgent):
    ...
    def __init__(self, *args: Any, **kwargs: Any) -> None:
        ...
        from opensteva.tools.channel_tools import ChannelSendTool
        from opensteva.tools.digest_collect import DigestCollectTool
        from opensteva.tools.proactive_tools import (
            CheckPermissionTool,
    ...
    def _build_system_prompt(self) -> str:
        user_md = _load_md_file(Path.home() / ".opensteva" / "USER.md")
        memory_md = _load_md_file(Path.home() / ".opensteva" / "MEMORY.md")
        now = datetime.now()
        context_block = ""
