...
harness — each is registered as a standard OpenSteva agent so the rest
...
(defaults to ``~/.opensteva/experiments/hybrid/``).
...
Items in parentheses are what the *paper* trains. These OpenSteva ports
...
`src/opensteva/evals/scorers/swebench_harness.py` (next to the existing
structural scorer).
...
```bash
src/opensteva/agents/hybrid/scripts/new_experiment.sh \
...
(`~/.opensteva/experiments/hybrid/docs/results.md`) at full N —
...
these OpenSteva cells should reproduce the harness numbers within
