...
from opensteva.agents.hybrid._base import (
    ANTHROPIC_WEB_SEARCH_TOOL,
    WEB_SEARCH_COST_PER_CALL,
    LocalCloudAgent,
)
from opensteva.agents.hybrid._prices import (
    PRICES,
    is_gpt5_family,
    supports_temperature,
)
from opensteva.agents.hybrid.mini_swe_agent import (
    _clone_repo,
    _extract_diff,
    run_swe_agent_loop,
)
...
# Map the orchestrator's `model` slot to a concrete OpenSteva worker spec.
...
def _call_tavily_search(query: str, max_results: int = 5) -> Tuple[str, int, int]:
    ...
    Token counts are reported as zero (no LLM was billed); the OpenSteva
...
_MODAL_APP_NAME = "opensteva-toolorchestra-sandbox"
