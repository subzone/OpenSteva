...
both forced by the OpenSteva environment and documented inline:
...
* in-tool correctness check — the original ``answer`` tool LLM-judges the
  prediction against the gold answer inside ``call_tool``. OpenSteva
...
def run_answer(
    ...
    *,
    ...
    in-tool LLM correctness check is dropped — OpenSteva scores
    downstream.
    """
...
def run_search(
    ...
    *,
    ...
    OpenSteva substitution for the missing FAISS wiki index).
    """
    prompt = (
        context_str.strip() + "\n\n"
        + f"Question: {problem}\nInstead of directly answering the question, "
