...
::
    ...
    python -m opensteva.agents.hybrid.runner --cell minions-gaia-qwen27b-opus-3
...
the registered agent, loads bench tasks via OpenSteva's existing dataset
...
existing rescore / dashboard scripts can read OpenSteva cells without
modification.
"""
...
from opensteva.agents.hybrid._energy import EnergyCollector
from opensteva.agents.hybrid._prompts import format_prompt as _format_prompt
...
DEFAULT_EXPERIMENTS_DIR = Path(
    os.environ.get(
        ...
        Path.home() / ".opensteva" / "experiments" / "hybrid",
    )
)
...
def _load_gaia_tasks(n: Optional[int]) -> List[Dict[str, Any]]:
    ...
    for rec in ds.iter_records():
        ...
        # OpenSteva prefixes with `gaia-`) so subsets keyed by the upstream
        # id round-trip.
        md = rec.metadata or {}
...
def _get_gaia_scorer():
    ...
    if _GAIA_SCORER is None:
        with _GAIA_SCORER_LOCK:
            if _GAIA_SCORER is None:
                from opensteva.evals.backends.steva_direct import (
                    StevaDirectBackend,
                )
                ...
                try:
                    backend = StevaDirectBackend(engine_key="cloud")
...
def _score_gaia(task: Dict[str, Any], answer: str) -> Dict[str, Any]:
    ...
    Uses the shared OpenSteva :class:`GAIAScorer`. The previous version
...
def _score_swebench(
    ...
    """
    from opensteva.evals.core.types import EvalRecord
    from opensteva.evals.scorers.swebench_harness import (
...
def _build_agent(cell: Dict[str, Any]):
    ...
    import opensteva.agents  # noqa: F401 — populate registry
...
def main(argv: Optional[List[str]] = None) -> int:
    p = argparse.ArgumentParser(
        prog="python -m opensteva.agents.hybrid.runner",
