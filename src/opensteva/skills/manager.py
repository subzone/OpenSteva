...
from opensteva.skills.dependency import validate_dependencies
from opensteva.skills.executor import SkillExecutor, SkillResult
from opensteva.skills.loader import discover_skills
...
class SkillManager:
    ...
    def __init__(
        ...
        *,
        ...
        if overlay_dir is None:
            ...
            # ~/.opensteva/learning/skills/ if config can't be loaded.
            try:
                ...
                pass
            if overlay_dir is None:
                overlay_dir = Path("~/.opensteva/learning/skills/").expanduser()
    ...
    def _load_overlays(self) -> None:
        ...
        examples under ``manifest.metadata.opensteva.few_shot``.

        Bad overlays are silently ignored — they should not break discovery.
        """
        from opensteva.skills.overlay import SkillOverlayLoader
        ...
        for name, manifest in self._skills.items():
            ...
            if overlay.few_shot:
                ...
                oj = dict(new_metadata.get("opensteva", {}) or {})
                oj["few_shot"] = list(overlay.few_shot)
                new_metadata["opensteva"] = oj
    ...
    def get_few_shot_examples(self) -> List[str]:
        ...
        Pulls from ``manifest.metadata.opensteva.few_shot`` for every
        registered skill.  Returns one formatted string per example.
        """
        examples: List[str] = []
        for name, manifest in self._skills.items():
            oj = manifest.metadata.get("opensteva", {}) if manifest.metadata else {}
    ...
    def discover_from_traces(
        ...
        *,
        ...
        ``~/.opensteva/skills/discovered/``).  Returns a list of dicts with
        ...
        if output_dir is None:
            output_dir = Path("~/.opensteva/skills/discovered/").expanduser()
    ...
    def _serialize_discovered_skill(name: str, skill: Any) -> str:
        ...
        lines.append('author = "opensteva (auto-discovered)"')
    ...
    def find_installed_paths(
        ...
        """
        if roots is None:
            roots = [Path("~/.opensteva/skills/").expanduser(), Path("./skills")]
        ...
        for root in roots:
            ...
            for candidate in root.rglob("*"):
                ...
                try:
                    from opensteva.skills.loader import load_skill_directory
