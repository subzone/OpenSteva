...
2. Tolerant pass — maps non-spec top-level fields to their canonical
   locations under metadata.opensteva.* via FIELD_MAPPING.
...
# dataclass attribute.  When target_kind is "opensteva_meta" the value is
# stored under manifest.metadata["opensteva"][attr].
FIELD_MAPPING: Dict[str, tuple[str, str]] = {
    ...
    "platforms": ("opensteva_meta", "platforms"),
    "prerequisites": ("opensteva_meta", "prerequisites"),
}
...
class SkillParser:
    ...
    def _build_manifest(
        ...
        captures unknown fields under metadata.opensteva.original_frontmatter,
        and merges metadata.opensteva.* into canonical fields.
        """
        ...
        if not isinstance(raw_metadata, dict):
            raw_metadata = {}
        # Initialize opensteva namespace
        oj_meta = dict(raw_metadata.get("opensteva") or {})
        ...
        for key, value in frontmatter.items():
            ...
            if key in FIELD_MAPPING:
                ...
                LOGGER.warning(
                    ...
                    "(value preserved in metadata.opensteva.original_frontmatter)",
                    key,
                    manifest.name,
                )

        # Merge metadata.opensteva.* into canonical fields (these override
        # top-level mappings since they are explicit OpenSteva-namespaced).
        for key in (
            "version",
            "author",
            "tags",
        ...
        # Preserve unmapped fields under metadata.opensteva.original_frontmatter
        if unmapped:
            oj_meta["original_frontmatter"] = unmapped

        # Stash the opensteva metadata block back
        if oj_meta:
            new_metadata = dict(raw_metadata)
            new_metadata["opensteva"] = oj_meta
