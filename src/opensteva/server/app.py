"""FastAPI application factory for the OpenSteva API server."""
...
from opensteva.server.analytics_routes import router as analytics_router
from opensteva.server.api_routes import include_all_routes
from opensteva.server.comparison import comparison_router
from opensteva.server.connectors_router import create_connectors_router
from opensteva.server.dashboard import dashboard_router
from opensteva.server.digest_routes import create_digest_router
from opensteva.server.research_router import router as research_router
from opensteva.server.routes import router
from opensteva.server.upload_router import router as upload_router
        ...
        for agent in mgr.list_agents():
            ...
            for b in bindings:
                ...
                if bridge and hasattr(bridge, "_channels"):
                    ...
                    from opensteva.server.channel_bridge import ChannelBridge
                    from opensteva.server.session_store import SessionStore
...
def create_app(
    ...
    *,
    ...
    config:
        Optional StevaConfig for other settings.
    """
    app = FastAPI(
        title="OpenSteva API",
        description="OpenAI-compatible API server for OpenSteva",
        version="0.1.0",
    )
    ...
    try:
        from opensteva.analytics import (
            AnalyticsClient,
            EventBridge,
    ...
    try:
        from opensteva.server.middleware import create_security_middleware
    ...
    if api_key:
        try:
            from opensteva.server.auth_middleware import AuthMiddleware
    ...
    if webhook_config:
        try:
            from opensteva.server.webhook_routes import (
