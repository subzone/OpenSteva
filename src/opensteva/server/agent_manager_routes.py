...
logger = logging.getLogger("opensteva.server.agent_manager")
...
def _resolve_memory_backend(config: Any) -> Any:
    ...
    if config is None or not getattr(config.agent, "context_from_memory", False):
        return None
    try:
        import opensteva.tools.storage  # noqa: F401
...
class _LightweightSystem:
    ...
    full StevaSystem (which picks a random model from Ollama)."""
...
def _make_lightweight_system(
    ...
    try:
        ...
        try:
            from opensteva.core.events import get_event_bus
            from opensteva.telemetry.instrumented_engine import (
                InstrumentedEngine,
            )
...
def _ensure_registries_populated() -> None:
    ...
    from opensteva.core.registry import ChannelRegistry, ToolRegistry

    # First, try a normal import (works if modules haven't been imported yet)
    try:
        import opensteva.channels  # noqa: F401
    except Exception:
        pass
    ...
    # Also try to import browser tools (not included in opensteva.tools.__init__)
    for _browser_mod in ("opensteva.tools.browser", "opensteva.tools.browser_axtree"):
        try:
    ...
    if not ChannelRegistry.keys():
        for mod_name in list(sys.modules):
            if mod_name.startswith("opensteva.channels.") and not mod_name.endswith(
                "_stubs"
            ):
                try:
    ...
    if not ToolRegistry.keys():
        for mod_name in list(sys.modules):
            if (
                mod_name.startswith("opensteva.tools.")
    ...
    if not any(ToolRegistry.contains(n) for n in _BROWSER_SUB_TOOLS):
        for _browser_mod in (
            "opensteva.tools.browser",
            "opensteva.tools.browser_axtree",
        ):
...
def build_tools_list() -> List[Dict[str, Any]]:
    ...
    from opensteva.core.credentials import TOOL_CREDENTIALS
    from opensteva.core.registry import ChannelRegistry, ToolRegistry
...
def _replay_history_messages(
    ...
    """
    from opensteva.core.types import Message, Role, ToolCall
...
def _build_deep_research_tools(
    ...
    from opensteva.tools.knowledge_sql import KnowledgeSQLTool
    from opensteva.tools.scan_chunks import ScanChunksTool
...
def _get_mcp_tools(app_state: Any) -> Tuple[List[Dict[str, Any]], Dict[str, Any]]:
    ...
    from opensteva.mcp.transport import StdioTransport, StreamableHTTPTransport
    from opensteva.tools.mcp_adapter import MCPToolProvider
    ...
    async def generate():
        ...
        while turns < max_turns:
            ...
            if tool_call_fragments and current_finish_reason == "tool_calls":
                ...
                from opensteva.core.types import ToolCall as MsgToolCall
                ...
                for tc in sorted_tcs:
                    ...
                    try:
                        ...
                        if mcp_adapter is not None:
                            ...
                            from opensteva.tools._stubs import (
...
def create_agent_manager_router(
    ...
    async def bind_channel(
        ...
        if req.channel_type == "imessage":
            ...
            if identifier:
                try:
                    from opensteva.channels.imessage_daemon import (
                        is_running,
                        run_daemon,
                    )
                    ...
                    if not is_running():
                        ...
                        if engine:
                            from opensteva.server.agent_manager_routes import (
                                _build_deep_research_tools,
                            )

                            tools = _build_deep_research_tools(engine=engine, model="")
                            if tools:
                                from opensteva.agents.deep_research import (
                                    DeepResearchAgent,
                                )
        ...
        if req.channel_type == "sendblue":
            ...
            if api_key_id and api_secret_key:
                try:
                    from opensteva.channels.sendblue import (
                        SendBlueChannel,
                    )
                    ...
                    if bridge and hasattr(bridge, "_channels"):
                        ...
                        from opensteva.server.channel_bridge import (
                            ChannelBridge,
                        )
                        from opensteva.server.session_store import (
                            SessionStore,
                        )
        ...
        if req.channel_type == "slack":
            ...
            if bot_token and app_token:
                try:
                    from opensteva.channels.slack_daemon import (
                        start_slack_daemon,
                    )
                    from opensteva.channels.slack_daemon import (
    ...
    def trigger_learning(agent_id: str):
        if not manager.get_agent(agent_id):
            raise HTTPException(status_code=404, detail="Agent not found")
        from opensteva.core.events import EventType, get_event_bus
    ...
    def list_traces(agent_id: str, limit: int = 20):
        ...
        try:
            ...
            config = load_config()
            store = TraceStore(config.traces.db_path or "~/.opensteva/traces.db")
    ...
    async def save_tool_credentials(tool_name: str, request: Request):
        from opensteva.core.credentials import save_credential
    ...
    def credential_status(tool_name: str):
        from opensteva.core.credentials import get_credential_status
    ...
    async def sendblue_test(request: Request):
        ...
        try:
            payload: Dict[str, str] = {
                "number": to_number,
                "content": (
                    "Hello from your OpenSteva agent! "
