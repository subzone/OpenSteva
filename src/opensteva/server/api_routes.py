...
async def list_agents(request: Request):
    ...
    registered = []
    try:
        import opensteva.agents  # noqa: F401 — side-effect registration
    ...
    running = []
    try:
        from opensteva.tools.agent_tools import _SPAWNED_AGENTS
...
async def create_agent(req: AgentCreateRequest, request: Request):
    """Spawn a new agent."""
    try:
        from opensteva.tools.agent_tools import AgentSpawnTool
...
async def kill_agent(agent_id: str, request: Request):
    ...
    try:
        from opensteva.tools.agent_tools import AgentKillTool
...
async def message_agent(agent_id: str, req: AgentMessageRequest, request: Request):
    ...
    try:
        from opensteva.tools.agent_tools import AgentSendTool
...
async def memory_index(req: MemoryIndexRequest, request: Request):
    ...
    try:
        ...
        from opensteva.tools.storage.ingest import ingest_path
...
async def list_skills(request: Request):
    ...
    try:
        from opensteva.core.registry import SkillRegistry
...
async def install_skill(request: Request):
    ...
    return {
        ...
        "message": "Use TOML files in ~/.opensteva/skills/",
    }
...
async def list_sessions(request: Request, limit: int = 20):
    ...
    try:
        from opensteva.sessions.store import SessionStore
...
async def prometheus_metrics(request: Request):
    ...
    try:
        ...
        lines = [
            "# HELP opensteva_requests_total Total requests processed",
            "# TYPE opensteva_requests_total counter",
            f"opensteva_requests_total {stats.get('total_requests', 0)}",
            "# HELP opensteva_tokens_total Total tokens generated",
            "# TYPE opensteva_tokens_total counter",
            f"opensteva_tokens_total {stats.get('total_tokens', 0)}",
            "# HELP opensteva_latency_avg_ms Average latency in milliseconds",
            "# TYPE opensteva_latency_avg_ms gauge",
            f"opensteva_latency_avg_ms {stats.get('avg_latency_ms', 0)}",
        ]
        from starlette.responses import PlainTextResponse
...
async def websocket_chat_stream(websocket: WebSocket):
    ...
    """
    from opensteva.server.auth_middleware import websocket_authorized
...
async def learning_stats(request: Request):
    ...
    # Skill discovery
    try:
        from opensteva.learning.agents.skill_discovery import SkillDiscovery
...
def include_all_routes(app) -> None:
    ...
    from opensteva.server.approval_routes import (
    ...
    try:
        if hasattr(app.state, "agent_manager") and app.state.agent_manager:
            from opensteva.server.agent_manager_routes import (  # noqa: PLC0415
    ...
    try:
        ...
        from opensteva.server.ws_bridge import create_ws_router
