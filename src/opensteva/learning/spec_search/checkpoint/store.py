...
A thin wrapper over a local git repository at ``<opensteva_home>/.git``.
...
from opensteva.learning.spec_search.storage.paths import (
    ConfigurationError,
    _find_source_root,
)
...
class CheckpointStore:
    ...
    root :
        ...
        ``~/.opensteva/`` (or ``$OPENJARVIS_HOME``); for tests it's a
    ...
    def init(self) -> None:
        ...
        Refuses to initialize if ``self.root`` is inside the OpenSteva source
        ...
        if source_root is not None:
            try:
                ...
                raise ConfigurationError(
                    ...
                    f"OpenSteva source tree ({source_root}). Refusing to "
        ...
        if not (self._root / ".git").exists():
            self._git("init", "-q")
            self._git("config", "user.email", "spec-search@opensteva.local")
            self._git("config", "user.name", "OpenSteva Spec Search")
        ...
        # Allow empty so init succeeds even on a brand-new opensteva home.
        self._git(
            "commit",
            "--allow-empty",
            "-q",
