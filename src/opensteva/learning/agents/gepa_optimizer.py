...
Uses GEPA's adapter pattern to bridge OpenSteva traces into GEPA's
...
from opensteva.core.config import GEPAOptimizerConfig
from opensteva.core.registry import LearningRegistry
from opensteva.learning._stubs import AgentLearningPolicy
...
class OpenStevaGEPAAdapter:
    """Implements GEPA's adapter protocol for OpenSteva agents.
    ...
    def optimize(self, trace_store: Any) -> Dict[str, Any]:
        ...
        if not HAS_GEPA:
            return {
                "status": "error",
                "reason": (
                    "gepa not installed (pip install 'opensteva[learning-gepa]')"
                ),
            }

        agent_name = self.config.agent_filter or "default"
        adapter = OpenStevaGEPAAdapter(trace_store, agent_name, self.config)
        adapter.load_traces()
...
__all__ = ["GEPAAgentOptimizer", "OpenStevaGEPAAdapter"]
