...
The wrapper adapts OpenSteva traces into ACE's
...
sidecar overlay under ``~/.opensteva/learning/ace/<task>/`` for the
...
from opensteva.core.config import ACEOptimizerConfig
...
def _default_save_dir(task_name: str) -> Path:
    return Path.home() / ".opensteva" / "learning" / "ace" / task_name


class _TraceDataProcessor:
    """Adapter that exposes OpenSteva traces in ACE's three-method API.
...
def _traces_to_samples(traces: List[Any]) -> List[Dict[str, Any]]:
    """Convert OpenSteva trace records to ACE's sample dict shape."""
