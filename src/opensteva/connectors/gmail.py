...
Uses OAuth 2.0 tokens stored locally (see :mod:`opensteva.connectors.oauth`).
...
from opensteva.connectors._stubs import BaseConnector, Document, SyncStatus
from opensteva.connectors.google_auth import (
    GoogleAuthError,
)
from opensteva.connectors.google_auth import (
    call_with_refresh as _call_with_refresh,
)
from opensteva.connectors.oauth import (
    GOOGLE_ALL_SCOPES,
...
class GmailConnector(BaseConnector):
    ...
    credentials_path:
        ...
        ``~/.opensteva/connectors/gmail.json``.
    """
    ...
    def is_connected(self) -> bool:
        ...
        token), which made `steva connect gmail` short-circuit with
    ...
    def _refresh_token(self) -> str:
        ...
        if not new:
            raise RuntimeError(
                "Gmail token refresh failed — re-run `steva connect gmail`"
            )
        return new
