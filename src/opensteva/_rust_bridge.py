"""Single point of contact between Python and the Rust ``opensteva_rust`` module.

Every Python module that wants to delegate to Rust should import helpers from
here rather than importing ``opensteva_rust`` directly.  The Rust backend is
...
def get_rust_module() -> _types.ModuleType:
    """Return the ``opensteva_rust`` module.
    ...
    """
    import opensteva_rust  # type: ignore[import-untyped]

    return opensteva_rust


def _detect_rust() -> bool:
    """Return ``True`` if the compiled ``opensteva_rust`` extension is importable.
...
def injection_result_from_json(json_str: str) -> object:
    ...
    from opensteva.security.injection_scanner import (
        InjectionScanResult,
    )
    from opensteva.security.types import ScanFinding, ThreatLevel
...
def retrieval_results_from_json(json_str: str) -> list:
    ...
    from opensteva.tools.storage._stubs import RetrievalResult
