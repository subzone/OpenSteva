...
User configuration lives at ``~/.opensteva/config.toml``.  ``load_config()``
...
if TYPE_CHECKING:
    # Only used by type-checkers (mypy/pyright) for the ``StevaConfig.mining``
...
DEFAULT_CONFIG_DIR = Path.home() / ".opensteva"
...
def recommend_model(hw: HardwareInfo, engine: str) -> str:
    ...
    """
    from opensteva.intelligence.model_catalog import BUILTIN_MODELS
...
class ACEOptimizerConfig:
    ...
    Install via ``pip install -e opensteva[learning-ace]`` once the
    ...
    # most OpenSteva users have credentials for.
    ...
    # Empty string defaults to ``~/.opensteva/learning/ace/<task>/``.
    save_dir: str = ""
    task_name: str = "opensteva"
...
class SkillsLearningConfig:
    ...
    overlay_dir: str = "~/.opensteva/learning/skills/"
...
class AgentConfig:
    ...
    default_system_prompt: str = (
        ...
        "hardware through OpenSteva. You are not a cloud service. Respond "
...
class AnalyticsConfig:
    ...
    usage events sent to the OpenSteva team's PostHog instance to
...
class WhatsAppBaileysChannelConfig:
    ...
    auth_dir: str = ""  # Defaults to ~/.opensteva/whatsapp_auth
    assistant_name: str = "Steva"
...
class SandboxConfig:
    ...
    enabled: bool = False
    image: str = "opensteva-sandbox:latest"
    timeout: int = 300
    workspace: str = ""
...
class SchedulerConfig:
    ...
    db_path: str = ""  # Defaults to ~/.opensteva/scheduler.db
...
class OperatorsConfig:
    ...
    enabled: bool = False
    manifests_dir: str = "~/.opensteva/operators"
...
class MemoryFilesConfig:
    ...
    soul_path: str = "~/.opensteva/SOUL.md"
    memory_path: str = "~/.opensteva/MEMORY.md"
    user_path: str = "~/.opensteva/USER.md"
    nudge_interval: int = 10
    persona_name: str = ""  # named persona dir under ~/.opensteva/personas/<name>/
...
class SkillsConfig:
    ...
    enabled: bool = True
    skills_dir: str = "~/.opensteva/skills/"
    active: str = "*"
    auto_discover: bool = True
    auto_sync: bool = False
    nudge_interval: int = 15
    index_repo: str = "https://github.com/opensteva/skill-index.git"
    index_dir: str = "~/.opensteva/skill-index/"
    max_depth: int = 5
...
@dataclass
class StevaConfig:
    """Top-level configuration for OpenSteva."""
...
# Sections that users may set via ``steva config set``.
# ``hardware`` is auto-detected and not user-settable.
_SETTABLE_SECTIONS = frozenset(StevaConfig.__dataclass_fields__.keys()) - {
    "hardware",
    "mining",
}
...
def validate_config_key(dotted_key: str) -> type:
    ...
    The function walks the ``StevaConfig`` dataclass hierarchy using
    ``dataclasses.fields()``.
    ...
    for i, part in enumerate(parts):
        ...
        if isinstance(fld_type, str):
            ...
            import opensteva.core.config as _cfg_mod
...
def _migrate_toml_data(data: Dict[str, Any], cfg: "StevaConfig") -> None:
...
def load_config(path: Optional[Path] = None) -> StevaConfig:
    ...
    path:
        ...
        otherwise ``~/.opensteva/config.toml``.
    """
    _ensure_config_dir()
    hw = detect_hardware()
    cfg = StevaConfig(hardware=hw)
...
# ---------------------------------------------------------------------------
# Default TOML generation (for ``steva init``)
# ---------------------------------------------------------------------------
...
def generate_default_toml(
    ...
    """Render a commented TOML string suitable for ``~/.opensteva/config.toml``."""
    engine = engine or recommend_engine(hw)
    model = recommend_model(hw, engine)
    gpu_line = ""
    if hw.gpu:
...
# enabled = false
# poll_interval = 60
# db_path = ""                # Defaults to ~/.opensteva/scheduler.db

# [channel.whatsapp_baileys]
# auth_dir = ""               # Defaults to ~/.opensteva/whatsapp_auth
# assistant_name = "Steva"
