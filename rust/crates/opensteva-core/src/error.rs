//! Error types for OpenSteva.

use thiserror::Error;

/// Top-level error type for all OpenSteva operations.
#[derive(Error, Debug)]
pub enum OpenStevaError {
...
mod tests {
    ...
    fn test_error_from_registry() {
        let e: OpenStevaError =
            RegistryError::DuplicateKey("foo".into(), "ToolRegistry").into();
        assert!(matches!(e, OpenStevaError::Registry(_)));
    }
