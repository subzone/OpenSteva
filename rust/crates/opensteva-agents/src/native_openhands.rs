...
use opensteva_core::{AgentContext, AgentResult, OpenStevaError, ToolResult};
use opensteva_tools::executor::ToolExecutor;
use regex::Regex;
...
impl<M: CompletionModel + 'static> OjAgent for NativeOpenHandsAgent<M> {
    ...
    async fn run(
    ...
    ) -> Result<AgentResult, OpenStevaError> {
            ...
            .map(|ctx| {
                ctx.conversation
                    ...
                    .filter_map(|m| match m.role {
                        opensteva_core::Role::User => {
                            Some(RigMessage::user(&m.content))
                        }
                        opensteva_core::Role::Assistant => {
        ...
        for turn in 1..=self.max_turns {
            let response = self
                ...
                .await
                .map_err(|e| {
                    OpenStevaError::Agent(opensteva_core::error::AgentError::Execution(
                        e.to_string(),
                    ))
                })?;
...
mod tests {
    ...
    type OpenHandsAgent = NativeOpenHandsAgent<RigModelAdapter<opensteva_engine::Engine>>;
