...
pub struct PyTraceStore {
    pub inner: Arc<opensteva_traces::TraceStore>,
}
...
impl PyTraceStore {
    ...
    fn new(path: Option<&str>) -> PyResult<Self> {
        let inner = match path {
            Some(p) => opensteva_traces::TraceStore::new(std::path::Path::new(p)),
            None => opensteva_traces::TraceStore::in_memory(),
        }
...
pub struct PyTraceCollector {
    inner: opensteva_traces::TraceCollector,
}
...
impl PyTraceCollector {
    ...
    fn new(store: &PyTraceStore) -> Self {
        Self {
            inner: opensteva_traces::TraceCollector::new(Arc::clone(&store.inner)),
        }
    }
...
pub struct PyTraceAnalyzer {
    store: Arc<opensteva_traces::TraceStore>,
}
...
impl PyTraceAnalyzer {
    ...
    fn stats(&self) -> PyResult<String> {
        let analyzer = opensteva_traces::TraceAnalyzer::new(&self.store);
        let stats = analyzer
            .overall_stats()
            .map_err(|e| PyErr::new::<pyo3::exceptions::PyRuntimeError, _>(e.to_string()))?;
