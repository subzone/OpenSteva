...
pub struct PyWorkflowGraph {
    inner: opensteva_workflow::WorkflowGraph,
}
...
impl PyWorkflowGraph {
    ...
    fn new(name: &str) -> Self {
        Self {
            inner: opensteva_workflow::WorkflowGraph::new(name),
        }
    }
    ...
    fn add_node(
        ...
        let nt = match node_type {
            "tool" => opensteva_workflow::NodeType::Tool,
            "condition" => opensteva_workflow::NodeType::Condition,
            "parallel" => opensteva_workflow::NodeType::Parallel,
            "loop" => opensteva_workflow::NodeType::Loop,
            "transform" => opensteva_workflow::NodeType::Transform,
            _ => opensteva_workflow::NodeType::Agent,
        };
        let node = opensteva_workflow::WorkflowNode {
            id: id.to_string(),
            node_type: nt,
    ...
    fn add_edge(&mut self, source: &str, target: &str) -> PyResult<()> {
        let edge = opensteva_workflow::WorkflowEdge {
...
pub struct PyWorkflowBuilder {
    inner: Option<opensteva_workflow::WorkflowBuilder>,
}
...
impl PyWorkflowBuilder {
    ...
    fn new(name: &str) -> Self {
        Self {
            inner: Some(opensteva_workflow::WorkflowBuilder::new(name)),
        }
    }
