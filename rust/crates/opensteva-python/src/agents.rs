...
use crate::RUNTIME;
use opensteva_agents::OjAgent;
use opensteva_engine::rig_adapter::RigModelAdapter;
use opensteva_engine::Engine;
use pyo3::prelude::*;
use std::sync::Arc;
...
enum AgentEnum {
    Simple(opensteva_agents::SimpleAgent<DefaultAdapter>),
    Orchestrator(opensteva_agents::OrchestratorAgent<DefaultAdapter>),
    NativeReAct(opensteva_agents::NativeReActAgent<DefaultAdapter>),
}
...
impl AgentEnum {
    ...
    async fn run(
        &self,
        input: &str,
        context: Option<&opensteva_core::AgentContext>,
    ) -> Result<opensteva_core::AgentResult, opensteva_core::OpenStevaError> {
        match self {
...
impl PySimpleAgent {
    ...
    fn new(
        ...
        let agent = opensteva_agents::SimpleAgent::new(adapter, system_prompt, temperature);
...
impl PyOrchestratorAgent {
    ...
    fn new(
        ...
        let adapter = make_adapter(engine_key, model)?;
        let executor = Arc::new(opensteva_tools::ToolExecutor::new(None, None));
        let agent = opensteva_agents::OrchestratorAgent::new(
...
impl PyNativeReActAgent {
    ...
    fn new(
        ...
        let adapter = make_adapter(engine_key, model)?;
        let executor = Arc::new(opensteva_tools::ToolExecutor::new(None, None));
        let agent = opensteva_agents::NativeReActAgent::new(
...
impl PyNativeOpenHandsAgent {
    ...
    fn new(
        ...
        let engine = opensteva_engine::get_engine_static(&config, Some(engine_key))
            .map_err(|e| PyErr::new::<pyo3::exceptions::PyRuntimeError, _>(e.to_string()))?;
        let adapter = opensteva_engine::rig_adapter::RigModelAdapter::new(
            Arc::new(engine),
            model.to_string(),
        );
        let executor = Arc::new(opensteva_tools::ToolExecutor::new(None, None));
        let agent = opensteva_agents::NativeOpenHandsAgent::new(
            adapter,
            executor,
            max_turns,
            temperature,
...
impl PyMonitorOperativeAgent {
    ...
    fn new(
        ...
        let mem_ext = match memory_extraction {
            "scratchpad" => opensteva_agents::MemoryExtraction::Scratchpad,
            "structured_json" => opensteva_agents::MemoryExtraction::StructuredJson,
            "none" => opensteva_agents::MemoryExtraction::None,
            _ => opensteva_agents::MemoryExtraction::CausalityGraph,
        };
        let obs_comp = match observation_compression {
            "truncate" => opensteva_agents::ObservationCompression::Truncate,
            "none" => opensteva_agents::ObservationCompression::None,
            _ => opensteva_agents::ObservationCompression::Summarize,
        };
        let ret_strat = match retrieval_strategy {
            "keyword" => opensteva_agents::RetrievalStrategy::Keyword,
            "semantic" => opensteva_agents::RetrievalStrategy::Semantic,
            "none" => opensteva_agents::RetrievalStrategy::None,
            _ => opensteva_agents::RetrievalStrategy::HybridWithSelfEval,
        };
        let task_dec = match task_decomposition {
            "monolithic" => opensteva_agents::TaskDecomposition::Monolithic,
            "hierarchical" => opensteva_agents::TaskDecomposition::Hierarchical,
            _ => opensteva_agents::TaskDecomposition::Phased,
        };

        let monitor_config = opensteva_agents::MonitorConfig {
        ...
        let agent = opensteva_agents::MonitorOperativeAgent::new(
            adapter,
            executor,
            max_turns,
            temperature,
...
pub struct PyLoopGuard {
    inner: opensteva_agents::LoopGuard,
}
...
impl PyLoopGuard {
    ...
    fn new(max_identical: usize, max_ping_pong: usize, poll_budget: usize) -> Self {
        Self {
            inner: opensteva_agents::LoopGuard::new(max_identical, max_ping_pong, poll_budget),
        }
    }
