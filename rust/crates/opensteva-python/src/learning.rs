...
use opensteva_learning::RouterPolicy;
use pyo3::prelude::*;

#[pyclass(name = "HeuristicRouter")]
pub struct PyHeuristicRouter {
    inner: opensteva_learning::HeuristicRouter,
}
...
impl PyHeuristicRouter {
    ...
    fn new(
        ...
        Self {
            inner: opensteva_learning::HeuristicRouter::new(
    ...
    fn select_model(&self, query: &str, has_code: bool, has_math: bool) -> String {
        let ctx = opensteva_core::RoutingContext {
...
pub struct PyBanditRouterPolicy {
    inner: opensteva_learning::BanditRouterPolicy,
}
...
impl PyBanditRouterPolicy {
    ...
    fn new(models: Vec<String>, strategy: &str) -> Self {
        let strat = match strategy {
            "ucb1" | "UCB1" => opensteva_learning::bandit::BanditStrategy::UCB1,
            _ => opensteva_learning::bandit::BanditStrategy::ThompsonSampling,
        };
        Self {
            inner: opensteva_learning::BanditRouterPolicy::new(models, strat),
        }
    }

    fn select_model(&self) -> String {
        let ctx = opensteva_core::RoutingContext::default();
        self.inner.select_model(&ctx)
    }
...
pub struct PyGRPORouterPolicy {
    inner: opensteva_learning::GRPORouterPolicy,
}
...
impl PyGRPORouterPolicy {
    ...
    fn new(models: Vec<String>, temperature: f64) -> Self {
        Self {
            inner: opensteva_learning::GRPORouterPolicy::new(models, temperature),
        }
    }
...
pub struct PyOptimizationStore {
    inner: parking_lot::Mutex<opensteva_learning::optimize::OptimizationStore>,
}
...
impl PyOptimizationStore {
    ...
    fn new(path: &str) -> PyResult<Self> {
        let store = if path == ":memory:" {
            opensteva_learning::optimize::OptimizationStore::in_memory()
        } else {
            opensteva_learning::optimize::OptimizationStore::open(path)
        };
        let store =
    ...
    fn save_trial(&self, run_id: &str, trial_json: &str) -> PyResult<()> {
        let trial: opensteva_learning::optimize::TrialResult =
...
pub struct PyLLMOptimizer {
    inner: opensteva_learning::optimize::LLMOptimizer,
}
...
impl PyLLMOptimizer {
    ...
    fn new(search_space_json: &str, optimizer_model: &str) -> PyResult<Self> {
        let search_space: opensteva_learning::optimize::SearchSpace =
            serde_json::from_str(search_space_json)
                .map_err(|e| PyErr::new::<pyo3::exceptions::PyValueError, _>(e.to_string()))?;
        let inner = opensteva_learning::optimize::LLMOptimizer::new(
            search_space,
    ...
    fn propose_next(&self, history_json: &str) -> PyResult<String> {
        let history: Vec<opensteva_learning::optimize::TrialResult> =
...
pub struct PySFTRouterPolicy {
    inner: opensteva_learning::SFTRouterPolicy,
}
...
impl PySFTRouterPolicy {
    ...
    fn new(min_samples: usize) -> Self {
        Self {
            inner: opensteva_learning::SFTRouterPolicy::new(min_samples),
        }
    }
    ...
    fn classify_query(query: &str) -> &'static str {
        opensteva_learning::SFTRouterPolicy::classify_query(query)
    }
...
pub struct PyHeuristicRewardFunction {
    inner: opensteva_learning::HeuristicRewardFunction,
}
...
impl PyHeuristicRewardFunction {
    ...
    fn new(
        ...
        Self {
            inner: opensteva_learning::HeuristicRewardFunction::new(
                weight_latency,
                weight_cost,
                weight_efficiency,
                max_latency,
...
pub struct PySkillDiscovery {
    inner: opensteva_learning::SkillDiscovery,
}
...
impl PySkillDiscovery {
    ...
    fn new(
        ...
        Self {
            inner: opensteva_learning::SkillDiscovery::new(
                min_frequency,
                min_sequence_length,
                max_sequence_length,
                min_outcome,
...
pub struct PyTraceDrivenPolicy {
    inner: opensteva_learning::TraceDrivenPolicy,
}
...
impl PyTraceDrivenPolicy {
    ...
    fn new(available_models: Vec<String>, default_model: &str, fallback_model: &str) -> Self {
        Self {
            inner: opensteva_learning::TraceDrivenPolicy::new(
                available_models,
                default_model.to_string(),
...
pub struct PyAgentAdvisorPolicy {
    inner: opensteva_learning::AgentAdvisorPolicy,
}
...
impl PyAgentAdvisorPolicy {
    ...
    fn new(max_traces: usize) -> Self {
        Self {
            inner: opensteva_learning::AgentAdvisorPolicy::new(max_traces),
        }
    }

    fn analyze_patterns(&self, traces_json: &str) -> PyResult<String> {
        let traces: Vec<opensteva_learning::TraceInfo> = serde_json::from_str(traces_json)
            .map_err(|e| PyErr::new::<pyo3::exceptions::PyValueError, _>(e.to_string()))?;
    ...
    fn classify(query: &str) -> &'static str {
        opensteva_learning::AgentAdvisorPolicy::classify(query)
    }
}
...
pub struct PyICLUpdaterPolicy {
    inner: opensteva_learning::ICLUpdaterPolicy,
}
...
impl PyICLUpdaterPolicy {
    ...
    fn new(min_score: f64, max_examples: usize, min_skill_occurrences: usize) -> Self {
        Self {
            inner: opensteva_learning::ICLUpdaterPolicy::new(
                min_score,
                max_examples,
...
pub struct PyTrainingDataMiner {
    inner: opensteva_learning::TrainingDataMiner,
}
...
impl PyTrainingDataMiner {
    ...
    fn new(min_quality: f64, min_samples_per_class: usize) -> Self {
        Self {
            inner: opensteva_learning::TrainingDataMiner::new(min_quality, min_samples_per_class),
        }
    }

    fn extract_sft_pairs(&self, traces_json: &str) -> PyResult<String> {
        let traces: Vec<opensteva_learning::MinerTraceData> = serde_json::from_str(traces_json)
            .map_err(|e| PyErr::new::<pyo3::exceptions::PyValueError, _>(e.to_string()))?;
...
pub struct PyAgentConfigEvolver {
    inner: opensteva_learning::AgentConfigEvolver,
}
...
impl PyAgentConfigEvolver {
    ...
    fn new(min_quality: f64) -> Self {
        Self {
            inner: opensteva_learning::AgentConfigEvolver::new(min_quality),
        }
    }

    fn analyze(&self, traces_json: &str) -> PyResult<String> {
        let traces: Vec<opensteva_learning::EvolutionTraceData> =
            serde_json::from_str(traces_json)
                .map_err(|e| PyErr::new::<pyo3::exceptions::PyValueError, _>(e.to_string()))?;
...
pub struct PyMultiObjectiveReward {
    inner: opensteva_learning::MultiObjectiveReward,
}
...
impl PyMultiObjectiveReward {
    ...
    fn new() -> Self {
        Self {
            inner: opensteva_learning::MultiObjectiveReward::new(
                opensteva_learning::RewardWeights::default(),
                opensteva_learning::Normalizers::default(),
            ),
        }
    }

    fn compute(&self, episode_json: &str) -> PyResult<f64> {
        let ep: opensteva_learning::Episode = serde_json::from_str(episode_json)
            .map_err(|e| PyErr::new::<pyo3::exceptions::PyValueError, _>(e.to_string()))?;
...
pub struct PyLearningOrchestrator {
    inner: opensteva_learning::LearningOrchestrator,
}
...
impl PyLearningOrchestrator {
    ...
    fn new(min_improvement: f64, min_sft_pairs: usize, min_quality: f64) -> Self {
        Self {
            inner: opensteva_learning::LearningOrchestrator::new(
                min_improvement,
                min_sft_pairs,
                min_quality,
            ),
