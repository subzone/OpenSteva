"""Tests for opensteva.evals.comparison.table_gen."""
...
from opensteva.evals.comparison.table_gen import (  # noqa: E402
    MixedCommitError,
    ResultsFrame,
    load_results,
    main,
...
class TestRenderBooktabs:
    def test_emits_valid_tabular(self) -> None:
        from opensteva.evals.comparison.table_gen import _render_booktabs

        df = pl.DataFrame(
            {
                "row": ["hermes", "opensteva"],
                "col1": [0.42, 0.55],
                "col2": [0.30, 0.40],
            }
        )
        ...
        assert "hermes" in fragment and "opensteva" in fragment
...
class TestT1Builder:
    def test_t1_builds_from_synthetic_results(self) -> None:
        from opensteva.evals.comparison.table_gen import (
            ResultsFrame,
            _build_t1,
        )
        ...
        assert "opensteva-distilled" in fragment.lower() or "OJ-distilled" in fragment
...
class TestT3to7Builders:
    ...
    def test_builder_emits_tabular(self, builder_name: str) -> None:
        import opensteva.evals.comparison.table_gen as m

        builder = getattr(m, builder_name)
        rows = []
        for fwk in ["hermes", "opensteva"]:
...
class TestTableBuilderRegistry:
    def test_registry_has_all_seven(self) -> None:
        from opensteva.evals.comparison.table_gen import _TABLE_BUILDERS
...
class TestTableGenCLI:
    def test_cli_writes_fragment_and_preview(self, tmp_path: Path) -> None:
        ...
        from opensteva.evals.comparison.table_gen import main
