...
from opensteva.cli._install_detect import InstallInfo
...
def _mock_info(kind: str = "pypi") -> InstallInfo:
    return InstallInfo(
        kind=kind,
        upgrade_command={
            "pypi": "pip install --upgrade opensteva",
            "uv-tool": "uv tool upgrade opensteva",
            "editable-git": "cd /tmp/repo && git pull && uv sync",
            "unknown": "pip install --upgrade opensteva",
        }[kind],
    )
...
def test_check_flag_prints_command_and_exits_clean():
    with patch(
        "opensteva.cli.self_update_cmd.detect_install",
        return_value=_mock_info("pypi"),
    ):
        runner = CliRunner()
        result = runner.invoke(self_update, ["--check"])
    assert result.exit_code == 0
    assert "pip install --upgrade opensteva" in result.output
...
def test_check_does_not_invoke_subprocess():
    with patch(
        "opensteva.cli.self_update_cmd.detect_install",
        return_value=_mock_info("pypi"),
    ), patch("opensteva.cli.self_update_cmd.subprocess.run") as mock_run:
...
def test_yes_skips_confirmation_and_runs():
    ...
    with patch(
        ...
        "opensteva.cli.self_update_cmd.subprocess.run",
    ...
    assert args[0] == ["pip", "install", "--upgrade", "opensteva"]
