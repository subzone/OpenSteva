...
from opensteva.cli._install_detect import InstallInfo, detect_install


def _patch_pkg_file(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> Path:
    """Point ``opensteva.__file__`` at ``tmp_path / opensteva / __init__.py``."""
    pkg_dir = tmp_path / "opensteva"
...
def test_uv_tool_install_detected(tmp_path, monkeypatch):
    fake = tmp_path / "share" / "uv" / "tools" / "opensteva" / "lib" / "python3.12"
    fake.mkdir(parents=True)
    _patch_pkg_file(fake, monkeypatch)

    info = detect_install()
    assert info.kind == "uv-tool"
    assert info.upgrade_command == "uv tool upgrade opensteva"
...
def test_pypi_install_detected(tmp_path, monkeypatch):
    ...
    assert info.upgrade_command == "pip install --upgrade opensteva"
...
def test_missing_opensteva_file_falls_back_to_pypi(monkeypatch):
    """opensteva unimportable / no __file__ — still get a sane default."""
    with patch("opensteva.cli._install_detect.Path") as mock_path:
