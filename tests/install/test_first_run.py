"""Tests for opensteva.cli._first_run.check_and_route."""
...
from opensteva.cli import _first_run
...
def test_passes_through_when_subcommand_present(tmp_opensteva_home: Path) -> None:
...
def test_routes_to_chat_when_config_exists(tmp_opensteva_home: Path) -> None:
    (tmp_opensteva_home / "config.toml").write_text('[engine]\ndefault = "ollama"\n')
...
def test_routes_to_init_when_no_config(tmp_opensteva_home: Path) -> None:
    ctx = _ctx_with_invocation(None)
    _first_run.check_and_route(ctx)
    assert ctx.invoke.called
    invoked_cmd = ctx.invoke.call_args[0][0]
...
def test_handles_missing_state_dir(tmp_path: Path, monkeypatch) -> None:
    """When ~/.opensteva doesn't exist at all, route to init."""
    fresh_home = tmp_path / "fresh"
    monkeypatch.setattr("opensteva.core.config.DEFAULT_CONFIG_DIR", fresh_home)
    monkeypatch.setattr(
        "opensteva.core.config.DEFAULT_CONFIG_PATH", fresh_home / "config.toml"
    )
    ctx = _ctx_with_invocation(None)
    _first_run.check_and_route(ctx)
    invoked_cmd = ctx.invoke.call_args[0][0]
...
def test_root_group_invokes_guard_on_bare_jarvis(
    ...
    monkeypatch.setattr("opensteva.cli._first_run.check_and_route", _recorder)
