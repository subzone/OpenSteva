"""Tests for opensteva.mining.apple_mps_pearl."""
...
_AVAIL = "opensteva.mining._install.pearl_packages_available"
...
def test_detect_requires_macos(hopper_hw):
    from opensteva.mining.apple_mps_pearl import AppleMpsPearlProvider
...
def test_detect_supported_when_torch_mps_available(apple_hw):
    ...
    with (
        patch("opensteva.mining._install.pearl_packages_available", return_value=True),
        patch(
            "opensteva.mining.apple_mps_pearl._torch_mps_available",
...
def test_start_uses_mps_miner_module(mps_config, tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("opensteva.mining.cpu_pearl._sidecar_path", lambda: sidecar)
    monkeypatch.setattr("opensteva.mining.cpu_pearl._log_dir", lambda: log_dir)
    monkeypatch.setattr("opensteva.mining.apple_mps_pearl._log_dir", lambda: log_dir)
    ...
    target = "opensteva.mining.apple_mps_pearl.PearlSubprocessLauncher"
    ...
    assert (
        ...
        == "opensteva.mining._mps_miner_loop_main"
    )
...
def test_mps_noisy_gemm_plain_proof_verifies_when_mps_available():
    ...
    from opensteva.mining._mps_miner_loop_main import (
        MpsNoisyGemmAdapter,
