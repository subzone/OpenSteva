...
class TestGEPAOptimizerOptimize:
    ...
    def test_no_gepa_reports_error(self) -> None:
        ...
        with patch.dict("sys.modules", {"gepa": None}):
            with patch(
                "opensteva.learning.agents.gepa_optimizer.HAS_GEPA",
                False,
            ):
                result = optimizer.optimize(mock_store)
...
class TestOpenStevaGEPAAdapter:
    def test_adapter_init(self) -> None:
        from opensteva.core.config import GEPAOptimizerConfig
        from opensteva.learning.agents.gepa_optimizer import (
            OpenStevaGEPAAdapter,
        )
