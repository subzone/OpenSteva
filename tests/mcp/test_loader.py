"""Regression tests for opensteva.mcp.loader.load_mcp_tools_from_config.
...
def _mock_mcp_stack():
    ...
    with patch("opensteva.mcp.client.MCPClient") as MockClient, patch(
        "opensteva.mcp.transport.StreamableHTTPTransport"
    ) as MockHttp, patch(
        "opensteva.mcp.transport.StdioTransport"
    ) as MockStdio, patch(
        "opensteva.tools.mcp_adapter.MCPToolProvider"
    ) as MockProvider:
